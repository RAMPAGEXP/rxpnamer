...
$BINARY tx wasm instantiate $CONTRACT_CODE "$RXPNAMER_INIT" --from "validator" --label "rxpnamer NFT nameservice" $TXFLAG --admin rxp16g2rahf5846rxzp3fwlswy08fz8ccuwk03k57y
RES=$?
...
MINT='{
  "mint": {
    "owner": "rxp16g2rahf5846rxzp3fwlswy08fz8ccuwk03k57y",
...
$BINARY tx wasm execute "$CONTRACT_ADDRESS" "$MINT" --from test-user $TXFLAG --amount 1000000urxpx

OLD_NFT_INFO=$(rxpd q wasm contract-state smart $CONTRACT_ADDRESS '{"all_nft_info": {"token_id": "nigeltufnel"}}' --output json)
...
# copy new
docker cp artifacts/rxpnamer.wasm $CONTAINER_NAME:/rxpnamer.wasm

# upload new wasm
NEW_CONTRACT_CODE=$($BINARY tx wasm store "/rxpnamer.wasm" --from validator $TXFLAG --output json | jq -r '.logs[0].events[-1].attributes[0].value')
...
echo "NEXT_PUBLIC_RXPNAMER_CODE_ID=$NEW_CONTRACT_CODE"
echo "NEXT_PUBLIC_RXPNAMER_ADDRESS=$NEW_CONTRACT_ADDRESS"
