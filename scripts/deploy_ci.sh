...
DEFAULT_DEV_ADDRESS="rxp16g2rahf5846rxzp3fwlswy08fz8ccuwk03k57y"
...
# CONTRACT_RES=$($BINARY tx wasm store "/rxpnamer.wasm" --from validator $TXFLAG --output json)
# echo $CONTRACT_RES
CONTRACT_CODE=$($BINARY tx wasm store "/rxpnamer.wasm" --from validator $TXFLAG --output json | jq -r '.logs[0].events[-1].attributes[0].value')
...
$BINARY tx wasm instantiate $CONTRACT_CODE "$RXPNAMER_INIT" --from "validator" --label "rxpnamer NFT nameservice" $TXFLAG --no-admin
RES=$?

# get contract addr
CONTRACT_ADDRESS=$($BINARY q wasm list-contract-by-code $CONTRACT_CODE --output json | jq -r '.contracts[-1]')

# provision rxp default user
...
echo "NEXT_PUBLIC_RXPNAMER_CODE_ID=$CONTRACT_CODE"
echo "NEXT_PUBLIC_RXPNAMER_ADDRESS=$CONTRACT_ADDRESS"
